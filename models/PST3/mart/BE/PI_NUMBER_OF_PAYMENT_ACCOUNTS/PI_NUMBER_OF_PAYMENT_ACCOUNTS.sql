    ...
    WHERE
        ...
        AND (
            ...
            OR ( ibis.ACCOUNT_STATUS = 'CLOSED'
               AND TIMESTAMP(ibis.ACCOUNT_UPDATED_AT) > TIMESTAMP(DATETIME( '{{end_date}}', '{{time_zone}}'))
             )
            )
        ...
        AND TIMESTAMP(ibis.ACCOUNT_CREATED_AT) <= TIMESTAMP(DATETIME( '{{end_date}}', '{{time_zone}}'))  --pt winter time
