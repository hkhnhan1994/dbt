...
{% set country_code = "BE" -%}
  ...
  JOIN {{ source('source_pst3_strp', 'D_FINANCIAL_PLATFORMS_DECRYPTED') }}
    AS fp ON pa.T_D_FINANCIAL_PLATFORM_DIM_KEY=fp.T_DIM_KEY
  JOIN {{ source('source_pst3_strp', 'D_FINANCIAL_INSTITUTIONS') }}
    AS fi ON fi.T_DIM_KEY = fp.T_D_FINANCIAL_INSTITUTION_DIM_KEY
  JOIN {{ source('source_pst3_strp', 'D_CONTRACT_INFO_CURRENT') }}
    AS c ON pa.T_DIM_KEY=c.T_D_PXG_PAYMENT_ACCOUNT_DIM_KEY
  JOIN {{ source('source_pst3_strp', 'D_APPLICATION_ACCOUNT_INFO_DECRYPTED') }}
    AS aa ON c.T_D_APPLICATION_ACCOUNT_DIM_KEY=aa.T_DIM_KEY
  JOIN {{ source('source_pst3_strp', 'D_APPLICATIONS_DECRYPTED') }}
    AS a ON aa.T_D_APPLICATION_DIM_KEY = a.T_DIM_KEY
  JOIN {{ source('source_pst3_strp', 'D_APPLICATION_OWNERS_DECRYPTED') }}
