...
{% set country_code = "BE" -%}
  ...
  SELECT
    ...
    LEFT JOIN {{ source('source_pst3_strp', 'D_ACCOUNT_TRANSACTION_CURRENT') }} AS d_account_transaction
      ON f_account_transactions.T_D_ACCOUNT_TRANSACTION_DIM_KEY = d_account_transaction.T_DIM_KEY
    LEFT JOIN {{ source('source_pst3_strp', 'D_IBIS_ACCOUNT_CURRENT') }} AS d_ibis_account
      ON f_account_transactions.T_D_IBIS_ACCOUNT_DIM_KEY = d_ibis_account.T_DIM_KEY
    INNER JOIN {{ source('source_pst3_strp', 'D_BANK_ACCOUNTS_DECRYPTED') }} AS d_bank_accounts
      ON d_ibis_account.T_D_BANK_ACCOUNT_DIM_KEY = d_bank_accounts.T_DIM_KEY
    LEFT JOIN {{ source('source_pst3_strp', 'D_BANK_ACCOUNTS_DECRYPTED') }} AS d_counterparty_bank_accounts
      ON d_counterparty_bank_accounts.T_DIM_KEY = f_account_transactions.T_COUNTERPARTY_BANK_ACCOUNT_DIM_KEY
    INNER JOIN {{ source('source_pst3_strp', 'F_ASPSP_PAYMENT_INITIATION') }} PI
      ON PI.T_D_ACCOUNT_TRANSACTION_DIM_KEY = d_account_transaction.T_DIM_KEY
    LEFT JOIN {{ source('source_pst3_strp', 'D_ASPSP_TPP') }} TPP
    ...
    WHERE
      ...
      AND f_account_transactions.TRANSACTION_TYPE = 'REGULAR'
       AND (
             d_account_transaction.TRANSACTION_BOOKING_DATE_AT >= TIMESTAMP(DATETIME( '{{begin_date}}', '{{time_zone}}'))
