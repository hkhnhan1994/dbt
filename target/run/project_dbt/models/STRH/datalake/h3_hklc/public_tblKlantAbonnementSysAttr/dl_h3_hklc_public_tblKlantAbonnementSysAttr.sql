    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_tblKlantAbonnementSysAttr`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`dev_lake_view_cmd`.`public_tblKlantAbonnementSysAttr` 
)
WHERE rn = 1
),
surrogate_key as(
