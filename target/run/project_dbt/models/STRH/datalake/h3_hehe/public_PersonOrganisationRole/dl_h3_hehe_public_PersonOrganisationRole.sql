    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hehe`.`public_PersonOrganisationRole`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`dev_lake_view_cmd`.`public_PersonOrganisationRole` 
)
WHERE rn = 1
),
surrogate_key as(
