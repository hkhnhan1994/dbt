    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_staging_view_cmd`.`D_ENTERPRISE`
...
pre_kal_sys AS (
    ...
    FROM (
      ...
      FROM `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_tblKlantSysAttr_current`
      WHERE systeemid = 1
      AND Waarde IS NOT NULL
      AND Waarde != ""
    ) AS  kal_tblant_sys
    JOIN `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_tblSysteemAttribuut_current` AS sys_at
  ...
  source_table AS (
      ...
      FROM `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_tblKlant_current` AS kal_klant
      LEFT JOIN `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_tblLand_current` AS kal_tblant ON kal_klant.LandID = kal_tblant.LandID
      LEFT JOIN kal_sys
        ON kal_sys.KlantID=kal_klant.KlantID
      LEFT JOIN `pj-bu-dw-data-sbx`.`dev_dwh_view_cmd`.`D_LEGAL_ENTITY_CURRENT`  AS leg
