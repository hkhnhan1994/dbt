    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_staging_view_cmd`.`D_LEGAL_ENTITY`
  ...
  WITH current_table AS (
    ...
    FROM  `pj-bu-dw-data-sbx`.`lake_view_cmd`.`public_requestcache`
  )
  SELECT * EXCEPT(rn)
  FROM current_table
  WHERE rn = 1
...
cast_respond_data AS(
  ...
  source_table AS (
    SELECT
      ...
      SHA256(CONCAT("HKVK", "$", CAST(re_ca.CacheKey AS STRING),CAST(re_ca.RequestCacheID AS STRING))) AS T_BUS_KEY,
      "HKVK" AS T_SOURCE,
  ...
  SELECT
    src.*,
    SHA256(FORMAT("%T", (SELECT AS STRUCT src.* EXCEPT(T_INGESTION_TIMESTAMP, T_DML_TYPE)))) AS T_ROW_HASH
