    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`dl_h3_hklc_hist_public_tblSysteemAttribuut`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`lake_view_cmd`.`public_tblSysteemAttribuut` 
)
WHERE rn = 1
),
surrogate_key as(
