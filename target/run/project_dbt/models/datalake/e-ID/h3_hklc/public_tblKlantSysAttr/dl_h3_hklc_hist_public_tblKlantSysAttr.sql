    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`dl_h3_hklc_hist_public_tblKlantSysAttr`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`lake_view_cmd`.`public_tblKlantSysAttr` 
)
WHERE rn = 1
),
surrogate_key as(
