    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hklc`.`public_aselect_loginhistory`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`dev_lake_view_cmd`.`public_aselect_loginhistory` 
)
WHERE rn = 1
),
surrogate_key as(
