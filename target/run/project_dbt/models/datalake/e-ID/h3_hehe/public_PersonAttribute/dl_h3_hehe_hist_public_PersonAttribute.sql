    ...
    create or replace table `pj-bu-dw-data-sbx`.`dev_dl_h3_hehe`.`dl_h3_hehe_hist_public_PersonAttribute`
...
FROM (
    ...
    FROM   `pj-bu-dw-data-sbx`.`dev_lake_view_cmd`.`public_PersonAttribute` 
)
WHERE rn = 1
),
surrogate_key as(
