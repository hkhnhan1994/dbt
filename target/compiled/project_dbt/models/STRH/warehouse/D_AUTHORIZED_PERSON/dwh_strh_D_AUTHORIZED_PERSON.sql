...
SELECT 
        ...
        (SELECT COALESCE(MAX(T_DIM_KEY), 1) FROM `pj-bu-dw-data-sbx`.`dev_dwh_view_cmd`.`D_AUTHORIZED_PERSON` ) + ROW_NUMBER() over (Order by surrogate_key.T_INGESTION_TIMESTAMP ASC) as T_DIM_KEY,
